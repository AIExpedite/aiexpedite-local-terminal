...
// that match the orphan-scanner allowlist (claude/codex/gemini). Used by
...
func scanViaPowerShell() []ProcessInfo {
	script := `$ErrorActionPreference = 'SilentlyContinue';` +
		` Get-CimInstance Win32_Process -Filter "Name='claude.exe' OR Name='codex.exe' OR Name='gemini.exe'"` +
...
func scanViaWMIC() []ProcessInfo {
	whereClause := `name="claude.exe" or name="codex.exe" or name="gemini.exe"`
	args := []string{
		"process",
		"where", whereClause,
