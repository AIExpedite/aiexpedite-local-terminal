...
- **Installers serialize on the destination.** Each channel holds a *different*
  ...
  (`~/Applications/.aixinstall.lock`) does: every writer of an installed bundle
  — the DMG install, the legacy `/Applications` relocation, and the updater's
  own swap and rollback — takes it before choosing a path, and placement is create-first (`renameatx_np(RENAME_EXCL)`) with
  ...
  different channel, which would leave this one uninstalled. The updater needs
  it because `swapDarwinBundles`' non-`RENAME_SWAP` fallback leaves the bundle
  briefly absent, and an installer looking in that gap would read the
  destination as free.
- **Startup recovery holds the destination lock.** It restores *and deletes* the
...
## The login item's name and icon

The LaunchAgent written by `writeDarwinLaunchAgentFor` declares
`AssociatedBundleIdentifiers` with the installed bundle's `CFBundleIdentifier`.
That key is what ties the background job to the app in System Settings ›
General › Login Items & Extensions. Without it macOS cannot tell which app owns
the job, so it groups the entry under the code-signing organization ("AI
Expedite Inc.") with a blank placeholder icon.

The identifier is read from the installed bundle, so each channel associates
with its own app, and it is omitted entirely when running outside a `.app` —
naming a bundle that is not installed puts the entry back to blank.

Existing installs pick this up when the plist is next rewritten (auto-update,
reinstall, or relocation), not retroactively.
